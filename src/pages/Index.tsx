...
const Index = () => {
  ...
  const handleGenerate = async () => {
    try {
      // Try to fetch from C++ backend first
      ...
      if (response.ok) {
        ...
        toast.success("Pattern generated by C++ backend!", {
          description: `${data.notes.length} notes in ${scale.replace("_", " ")}`,
        });
        return;
      }
    } catch (error) {
      // Backend not available, fall back to client-side generation
      console.log("C++ backend not available, using client-side generation");
    }
    ...
    setCurrentNoteIndex(-1);
    toast.info("Pattern generated client-side", {
      description: `${pattern.notes.length} notes in ${scale.replace("_", " ")} (C++ backend offline)`,
    });
  };
